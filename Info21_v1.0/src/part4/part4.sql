...
DECLARE
    ...
    drop_query     TEXT;
BEGIN
    FOR cur_table_name IN (SELECT table_name
                           FROM information_schema.tables
                           WHERE table_schema = 'public'
                             AND table_name ILIKE prefix_to_drop || '%')
        LOOP
...
CREATE TRIGGER trg_before_insert_test_table1
    BEFORE INSERT
    ON public.test_table1
    FOR EACH ROW
EXECUTE FUNCTION trg_before_insert_test_table1();
...
CREATE TRIGGER trg_before_update_test_table2
    BEFORE UPDATE
    ON public.test_table2
    FOR EACH ROW
EXECUTE FUNCTION trg_before_update_test_table2();
